...
static void console_ensure_inited()
{
    if (!console_is_inited())
        ...
        // Centred, and lifted three lines clear of the bottom. The default
        // bottom-left position sits directly underneath the persistent overlay's
        // left plate, which hid the script output completely.
        // 40 chars, not 25. The screen is 45 characters wide and the default
        // was clipping diagnostics mid-word - "Fail to load zebra.flt: open
        // error" arrived as "Fail to load zebra.flt", which reads as a
        // different fault entirely.
        console_init(5, 40,
                     ((camera_screen.width/FONT_WIDTH) - 40) / 2,
                     MAX_CONSOLE_LINES - 5 - 3);
#else
