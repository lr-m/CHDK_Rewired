...
void cpu_init_emu_cpu_state(struct gb_state *s) {
    s->emu_cpu_state = ucalloc(sizeof(struct emu_cpu_state));
...
void cpu_reset_state(struct gb_state *s) {
    ...
    /* CHDK port: NULL-guarded. Upstream can assume every one of these is
     * allocated, because state_new_from_rom() allocates all of them. This port
     * does not: the camera cartridge owns its own 128K of SRAM inside
     * gbc_cam_t, so mem_EXTRAM is deliberately left NULL for it while
     * mem_num_banks_extram still reads 16 from the ROM header.
     *
     * Unguarded, that is memset(NULL, 0, 0x20000) - and on DryOS there is no
     * MMU to catch it, so it silently flattens the bottom 128K of RAM
     * (exception vectors included) and the body wedges and resets before the
     * emulator has run a single instruction. Nothing reaches the screen, which
     * is what made it look like an emulator bug rather than a setup one. */
    if (s->mem_WRAM)
...
static void cpu_handle_interrupts(struct gb_state *s) {
    ...
    /* CHDK port: nothing pending is the overwhelmingly common case, and both
     * branches below are no-ops when interrupts == 0. Returning here skips a
     * five-iteration bit scan on almost every instruction. */
    if (!interrupts)
        return;
...
/* CHDK port: cycles-per-tick, precomputed.
 *
 * Upstream recomputes these with integer division on every instruction:
 *   freq / GB_DIV_FREQ, and freq / GB_TIMA_FREQS[TAC & 3]
 * ARM946 has no divide instruction, so each one is a call into
 * __aeabi_uidiv - tens of cycles, on the hottest path in the emulator, to
 * recompute one of eight constants.
 *
 * [0] is normal speed, [1] is CGB double speed.
 *   4194304/16384 = 256      8388608/16384 = 512
 *   and the four TIMA rates {4096, 262144, 65536, 16384} likewise.
 */
static const u32 gb_div_cycles[2]      = { 2 * GB_FREQ / 16384,
                                           GB_FREQ / 16384 };
static const u32 gb_tima_cycles[2][4]  = {
    { 2 * GB_FREQ / 4096, 2 * GB_FREQ / 262144,
      2 * GB_FREQ / 65536, 2 * GB_FREQ / 16384 },
    {     GB_FREQ / 4096,     GB_FREQ / 262144,
          GB_FREQ / 65536,     GB_FREQ / 16384 },
};

void cpu_timers_step(struct gb_state *s) {
    u32 sp_idx = s->double_speed ? 1 : 0;
    u32 div_cycles_per_tick = gb_div_cycles[sp_idx];
    s->io_timer_DIV_cycles += s->emu_state->last_op_cycles;
    if (s->io_timer_DIV_cycles >= div_cycles_per_tick) {
        /* Subtract rather than modulo: another division upstream, and one
         * that cannot be needed - last_op_cycles tops out around 24 and the
         * smallest tick here is 32, so the loop runs once. */
        do {
            s->io_timer_DIV_cycles -= div_cycles_per_tick;
        } while (s->io_timer_DIV_cycles >= div_cycles_per_tick);
        s->io_timer_DIV++;
    }

    if (s->io_timer_TAC & (1<<2)) { /* Timer enable */
        s->io_timer_TIMA_cycles += s->emu_state->last_op_cycles;
        u32 timer_cycles_per_tick = gb_tima_cycles[sp_idx][s->io_timer_TAC & 0x3];
        if (s->io_timer_TIMA_cycles >= timer_cycles_per_tick) {
            do {
                s->io_timer_TIMA_cycles -= timer_cycles_per_tick;
            } while (s->io_timer_TIMA_cycles >= timer_cycles_per_tick);
            s->io_timer_TIMA++;
...
/* Dispatch table generated by tools/gbc_mkdispatch.py - do not
 * hand-edit. Maps an opcode to the index of the first rule in the
 * original if/else chain that matched it. 0xff = unknown opcode. */
static const unsigned char gb_rule_cb[256] = {
...
/* CHDK port: opcode prefetch, filled by cpu_step().
 *
 * cpu_step() has to read the opcode anyway to look up its cycle count, and
 * then both of these read the very same byte a second time. A CB-prefixed
 * instruction was costing four fetches where two will do. Handing the already
 * fetched bytes over here removes one mmu_read per instruction - it was 68
 * million reads against 39 million instructions - without touching any of the
 * generated dispatch bodies below, which still refer to `op` exactly as
 * tools/gbc_mkdispatch.py emitted them.
 *
 * Safe because each of these functions has exactly one call site: this one
 * from the 0xcb rule, and cpu_do_instruction() from cpu_step() itself. */
static u8 gb_prefetch_op;
static u8 gb_prefetch_cb;

static void cpu_do_cb_instruction(struct gb_state *s) {

    u8 op = gb_prefetch_cb;
    s->pc++;
    switch (gb_rule_cb[op]) {
    case 0: { /* M(op, 0x00, 0xf8) */
 /* RLC reg8 */
        u8 *reg = REG8(0);
 ...
 * hand-edit. Maps an opcode to the index of the first rule in the
 * original if/else chain that matched it. 0xff = unknown opcode. */
static const unsigned char gb_rule_instruction[256] = {
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x04, 0x05, 0x06, 0x0c,
    0x0d, 0x01, 0x0e, 0x03, 0x04, 0x05, 0x06, 0x0f, 0x10, 0x09, 0x11, 0x0b, 0x04, 0x05, 0x06, 0x12,
    0x13, 0x01, 0x14, 0x03, 0x04, 0x05, 0x06, 0x15, 0x13, 0x09, 0x16, 0x0b, 0x04, 0x05, 0x06, 0x17,
    0x13, 0x01, 0x18, 0x03, 0x04, 0x05, 0x06, 0x19, 0x13, 0x09, 0x1a, 0x0b, 0x04, 0x05, 0x06, 0x1b,
    ...
    0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x26, 0x2e, 0x28, 0x3a, 0x2a, 0x2f, 0x30, 0x2d,
    0x26, 0x27, 0x28, 0xff, 0x2a, 0x2b, 0x31, 0x2d, 0x26, 0x32, 0x28, 0xff, 0x2a, 0xff, 0x33, 0x2d,
    0x34, 0x27, 0x35, 0xff, 0xff, 0x2b, 0x36, 0x2d, 0x37, 0x38, 0x39, 0xff, 0xff, 0xff, 0x3b, 0x2d,
    0x3c, 0x27, 0x3d, 0x3e, 0xff, 0x2b, 0x3f, 0x2d, 0x40, 0x41, 0x42, 0x43, 0xff, 0xff, 0x44, 0x2d,
};

static void cpu_do_instruction(struct gb_state *s) {

    u8 op = gb_prefetch_op;      /* CHDK port: prefetched, see above */
    s->pc++;
    switch (gb_rule_instruction[op]) {
    case 0: { /* M(op, 0x00, 0xff) */
 /* NOP */
    } break;
    case 1: { /* M(op, 0x01, 0xcf) */
 /* LD reg16, u16 */
        u16 *dst = REG16(4);
        *dst = IMM16;
        s->pc += 2;
    } break;
    case 2: { /* M(op, 0x02, 0xff) */
 /* LD (BC), A */
        mmu_write(s, BC, A);
    } break;
    case 3: { /* M(op, 0x03, 0xcf) */
 /* INC reg16 */
        u16 *reg = REG16(4);
        *reg += 1;
    } break;
    case 4: { /* M(op, 0x04, 0xc7) */
 /* INC reg8 */
        u8* reg = REG8(3);
        u8 val = reg ? *reg : mem(HL);
        u8 res = val + 1;
 ...
 /* LD reg8, imm8 */
    ...
    case 7: { /* M(op, 0x07, 0xff) */
 /* RLCA */
        u8 res = (A << 1) | (A >> 7);
        F = (A >> 7) ? FLAG_C : 0;
        A = res;
    } break;
    case 8: { /* M(op, 0x08, 0xff) */
 /* LD (imm16), SP */
        mmu_write16(s, IMM16, s->sp);
        s->pc += 2;
    } break;
    case 9: { /* M(op, 0x09, 0xcf) */
 /* ADD HL, reg16 */
        u16 *src = REG16(4);
        u32 tmp = HL + *src;
        NF = 0;
    ...
    case 10: { /* M(op, 0x0a, 0xff) */
 /* LD A, (BC) */
        A = mem(BC);
    } break;
    case 11: { /* M(op, 0x0b, 0xcf) */
 /* DEC reg16 */
        u16 *reg = REG16(4);
        *reg -= 1;
    } break;
    case 12: { /* M(op, 0x0f, 0xff) */
 /* RRCA */
        F = (A & 1) ? FLAG_C : 0;
        A = (A >> 1) | ((A & 1) << 7);
    } break;
    case 13: { /* M(op, 0x10, 0xff) */
 /* STOP */
        //s->halt_for_interrupts = 1;
    } break;
    case 14: { /* M(op, 0x12, 0xff) */
 /* LD (DE), A */
        mmu_write(s, DE, A);
    } break;
    case 15: { /* M(op, 0x17, 0xff) */
 /* RLA */
        u8 res = A << 1 | (CF ? 1 : 0);
        F = (A & (1 << 7)) ? FLAG_C : 0;
        A = res;
    } break;
    case 16: { /* M(op, 0x18, 0xff) */
 /* JR off8 */
        s->pc += (s8)IMM8 + 1;
    } break;
    case 17: { /* M(op, 0x1a, 0xff) */
 /* LD A, (DE) */
        A = mem(DE);
    } break;
    case 18: { /* M(op, 0x1f, 0xff) */
 /* RRA */
        u8 res = (A >> 1) | (CF << 7);
        ZF = 0;
        NF = 0;
 ...
 /* JR cond, off8 */
    ...
    case 20: { /* M(op, 0x22, 0xff) */
 /* LDI (HL), A */
        mmu_write(s, HL, A);
        HL++;
    } break;
    case 21: { /* M(op, 0x27, 0xff) */
 /* DAA */
    ...
    case 22: { /* M(op, 0x2a, 0xff) */
 /* LDI A, (HL) */
        A = mmu_read(s, HL);
        HL++;
    } break;
    case 23: { /* M(op, 0x2f, 0xff) */
 /* CPL */
        A = ~A;
        NF = 1;
        HF = 1;
    } break;
    case 24: { /* M(op, 0x32, 0xff) */
 /* LDD (HL), A */
        mmu_write(s, HL, A);
        HL--;
    } break;
    case 25: { /* M(op, 0x37, 0xff) */
 /* SCF */
        NF = 0;
        HF = 0;
        CF = 1;
    } break;
    case 26: { /* M(op, 0x3a, 0xff) */
 /* LDD A, (HL) */
        A = mmu_read(s, HL);
        HL--;
    } break;
    case 27: { /* M(op, 0x3f, 0xff) */
 /* CCF */
        CF = CF ? 0 : 1;
        NF = 0;
        HF = 0;
    } break;
    case 28: { /* M(op, 0x76, 0xff) */
 /* HALT */
        s->halt_for_interrupts = 1;
    } break;
    case 29: { /* M(op, 0x40, 0xc0) */
 /* LD reg8, reg8 */
        u8* src = REG8(0);
        u8* dst = REG8(3);
 ...
 /* RET cond */
    ...
    case 39: { /* M(op, 0xc1, 0xcf) */
 /* POP reg16 */
        u16 *dst = REG16S(4);
        *dst = mmu_pop16(s);
        F = F & 0xf0;
    } break;
    case 40: { /* M(op, 0xc2, 0xe7) */
 /* JP cond, imm16 */
        u8 flag = (op >> 3) & 3;
        if (((F & flagmasks[flag]) ? 1 : 0) == (flag & 1))
            s->pc = IMM16;
    ...
    case 41: { /* M(op, 0xc3, 0xff) */
 /* JP imm16 */
        s->pc = IMM16;
    } break;
    case 42: { /* M(op, 0xc4, 0xe7) */
 /* CALL cond, imm16 */
        u16 dst = IMM16;
        s->pc += 2;
        u8 flag = (op >> 3) & 3;
    ...
    case 43: { /* M(op, 0xc5, 0xcf) */
 /* PUSH reg16 */
        u16 *src = REG16S(4);
        mmu_push16(s,*src);
    } break;
    case 44: { /* M(op, 0xc6, 0xff) */
 /* ADD A, imm8 */
        u16 res = A + IMM8;
        ZF = (u8)res == 0;
        NF = 0;
    ...
    case 45: { /* M(op, 0xc7, 0xc7) */
 /* RST imm8 */
        mmu_push16(s, s->pc);
        s->pc = ((op >> 3) & 7) * 8;
    } break;
    case 46: { /* M(op, 0xc9, 0xff) */
 /* RET */
        s->pc = mmu_pop16(s);
    } break;
    case 47: { /* M(op, 0xcd, 0xff) */
 /* CALL imm16 */
        u16 dst = IMM16;
        mmu_push16(s, s->pc + 2);
        s->pc = dst;
    } break;
    case 48: { /* M(op, 0xce, 0xff) */
 /* ADC imm8 */
        u16 res = A + IMM8 + CF;
        ZF = (u8)res == 0;
        NF = 0;
 ...
 /* SUB imm8 */
    ...
    case 50: { /* M(op, 0xd9, 0xff) */
 /* RETI */
        s->pc = mmu_pop16(s);
        s->interrupts_master_enabled = 1;
    } break;
    case 51: { /* M(op, 0xde, 0xff) */
 /* SBC imm8 */
        u8 res = A - IMM8 - CF;
        ZF = res == 0;
        NF = 1;
    ...
    case 52: { /* M(op, 0xe0, 0xff) */
 /* LD (0xff00 + imm8), A */
        mmu_write(s, 0xff00 + IMM8, A);
        s->pc++;
    } break;
    case 53: { /* M(op, 0xe2, 0xff) */
 /* LD (0xff00 + C), A */
        mmu_write(s, 0xff00 + C, A);
    } break;
    case 54: { /* M(op, 0xe6, 0xff) */
 /* AND imm8 */
        A = A & IMM8;
        s->pc++;
        ZF = A == 0;
 ...
 /* ADD SP, imm8s */
    ...
    case 56: { /* M(op, 0xe9, 0xff) */
 /* LD PC, HL (or JP (HL) ) */
        s->pc = HL;
    } break;
    case 57: { /* M(op, 0xea, 0xff) */
 /* LD (imm16), A */
        mmu_write(s, IMM16, A);
        s->pc += 2;
    } break;
    case 58: { /* M(op, 0xcb, 0xff) */
 /* CB-prefixed extended instructions */
        return cpu_do_cb_instruction(s);
    } break;
    case 59: { /* M(op, 0xee, 0xff) */
 /* XOR imm8 */
        A ^= IMM8;
        s->pc++;
        F = A ? 0 : FLAG_Z;
    } break;
    case 60: { /* M(op, 0xf0, 0xff) */
 /* LD A, (0xff00 + imm8) */
        A = mmu_read(s, 0xff00 + IMM8);
        s->pc++;
    } break;
    case 61: { /* M(op, 0xf2, 0xff) */
 /* LD A, (0xff00 + C) */
        A = mmu_read(s, 0xff00 + C);
    } break;
    case 62: { /* M(op, 0xf3, 0xff) */
 /* DI */
        s->interrupts_master_enabled = 0;
    } break;
    case 63: { /* M(op, 0xf6, 0xff) */
 /* OR imm8 */
        A |= IMM8;
        F = A ? 0 : FLAG_Z;
        s->pc++;
    } break;
    case 64: { /* M(op, 0xf8, 0xff) */
 /* LD HL, SP + imm8 */
    ...
    case 65: { /* M(op, 0xf9, 0xff) */
 /* LD SP, HL */
        s->sp = HL;
    } break;
    case 66: { /* M(op, 0xfa, 0xff) */
 /* LD A, (imm16) */
        A = mmu_read(s, IMM16);
        s->pc += 2;
    } break;
    case 67: { /* M(op, 0xfb, 0xff) */
 /* EI */
        s->interrupts_master_enabled = 1;
    } break;
    case 68: { /* M(op, 0xfe, 0xff) */
 /* CP imm8 */
        u8 n = IMM8;
        ZF = A == n;
        NF = 1;
...
void cpu_step(struct gb_state *s) {
    ...
    op = mmu_read(s, s->pc);
    gb_prefetch_op = op;                    /* CHDK port: see the prefetch note */
    s->emu_state->last_op_cycles = cycles_per_instruction[op];
    if (op == 0xcb) {
        u8 cbop = mmu_read(s, s->pc + 1);
        gb_prefetch_cb = cbop;
        s->emu_state->last_op_cycles = cycles_per_instruction_cb[cbop];
    }
    ...
    /* CHDK port: one compare instead of up to six.
     *
     * These three range tests are a diagnostic that never fires on a working
     * ROM, but upstream pays for all of them on every instruction - and on
     * ARM946 an untaken branch is not free, there is no branch predictor. Every
     * region they care about is at or above 0x8000, and the program counter
     * spends nearly all its time in ROM below that, so one compare rejects the
     * common case and the detailed tests only run when something is genuinely
     * unusual. */
    if (s->pc >= 0x8000) {
        if (s->pc < 0xa000)
