...
#include "hwdefs.h"
#include "debugger.h"
#include "gbc_hooks.h"      /* CHDK port */

/* CHDK port: was #if 1. The printf inside these is stubbed out by gbc_port.h,
 * but the guard around it is not - so every mmu_read and mmu_write was
 * chasing s->emu_state and branching on dbg_print_mmu to decide whether to
 * call a function that does nothing. On the hottest path in the emulator.
 * Set back to 1 only alongside a real printf. */
#if 0
...
#define MMU_DEBUG_HDMA(...)     printf(__VA_ARGS__)
#else
#define MMU_DEBUG_W(...)
#define MMU_DEBUG_R(...)
#define MMU_DEBUG_HDMA(...)
#endif
...
/* CHDK port: compiled out. These are upstream's debug assertions and five of
 * them sit on the read path, each a live compare-and-branch on the hottest
 * code in the emulator. What they check - bank indices being in range - the
 * pager already enforces by masking (gbc_rom_bank), so a failure cannot
 * produce an out-of-bounds access, only a wrong-looking picture. Restore this
 * body if the core is ever being debugged again. */
#define mmu_assert(cond) do { (void)0; } while (0)

/* CHDK port: defined below, next to mmu_read. */
...
static void mmu_hdma_start(struct gb_state *s, u8 lenmode) {
    ...
    /* CHDK port: gated. printf is a no-op macro on the camera (gbc_port.h) so
     * this cost nothing there, but on the host builds it is a real printf on a
     * path Pokemon Crystal takes constantly - enough to dominate its benchmark
     * and make timing comparisons meaningless. */
    MMU_DEBUG_HDMA("HDMA @%.2x:%.4x %.4x -> %.4x, blocks=%.2x mode_hblank=%d\n",
...
void mmu_write(struct gb_state *s, u16 location, u8 value) {
    ...
    /* CHDK port: the camera cartridge owns every mapper range and a000-bfff.
     * Taken here rather than case by case because none of its registers line
     * up with the MBCs below - 4000-5fff is a mode select, not a RAM bank, and
     * a000 is a sensor register file that triggers a capture on write. */
    if (s->mbc == GBC_MBC_CAM &&
        (location < 0x6000 || (location >= 0xa000 && location < 0xc000))) {
        gbc_hook_cam_write(s, location, value);
        return;
    }
    ...
    case 0xb000:
        if (s->mbc == 1) {
            ...
            if (s->mem_mbc3_extram_rtc_select < 0x04) {
                ...
                /* CHDK port: -0x08. mem_RTC is u8[5] and the selector runs
                 * 0x08..0x0c, so upstream indexes 3 to 7 bytes past the end of
                 * the array. What follows it in struct gb_state is mem_latch_rtc,
                 * gb_type and `int mbc` - so an MBC3 cartridge touching its clock
                 * quietly rewrites its own mapper type mid-run. */
                s->mem_RTC[s->mem_mbc3_extram_rtc_select - 0x08] = value;
            else
...
/* CHDK port: rebuild the read page table. Must be called after anything that
 * changes what an address maps to - a ROM/VRAM/WRAM bank switch, leaving the
 * BIOS overlay, or bringing the machine up.
 *
 * Only regions that are a plain window onto memory are mapped. a000-bfff is
 * left unmapped on purpose: on this port it is either cartridge RAM behind an
 * enable flag or the camera's register file, and reading it has side effects
 * that the general path handles. Likewise e000-ffff (echo, OAM, I/O, HRAM).
 *
 * The switchable ROM window is resolved through the pager here and nowhere
 * else, which is what makes the cached pointer safe: with 4000-7fff mapped,
 * nothing else asks the pager for a bank, so no eviction can invalidate the
 * pointer between remaps. */
void gbc_mmu_remap(struct gb_state *s)
{
    ...
    /* While the BIOS is overlaid at 0000-00ff the low page is not a straight
     * window onto the cartridge, so everything takes the slow path. */
    if (s->in_bios || !r)
        return;

    p = gbc_rom_bank(r, 0);                     /* pinned */
    for (i = 0x00; i < 0x40; i++)
        s->rdmap[i] = p + ((i - 0x00) << 8);

    {
        int bank;
        if (s->mbc == GBC_MBC_CAM) {
            bank = gbc_hook_cam_rom_bank(s);
        } else {
            bank = s->mem_bank_rom;
            if (s->mbc == 1 && s->mem_mbc1_romram_select == 0)
                bank |= s->mem_mbc1_rombankupper << 5;
        }
        if (s->mem_num_banks_rom > 0)
            bank &= s->mem_num_banks_rom - 1;

        p = gbc_rom_bank(r, bank);
        for (i = 0x40; i < 0x80; i++)
            s->rdmap[i] = p + ((i - 0x40) << 8);
    }

    if (s->mem_VRAM) {
        p = s->mem_VRAM + s->mem_bank_vram * VRAM_BANKSIZE;
        for (i = 0x80; i < 0xa0; i++)
            s->rdmap[i] = p + ((i - 0x80) << 8);
    }

    if (s->mem_WRAM) {
        for (i = 0xc0; i < 0xd0; i++)
            s->rdmap[i] = s->mem_WRAM + ((i - 0xc0) << 8);
        p = s->mem_WRAM + s->mem_bank_wram * WRAM_BANKSIZE;
        for (i = 0xd0; i < 0xe0; i++)
            s->rdmap[i] = p + ((i - 0xd0) << 8);
    }
}

u8 mmu_read(struct gb_state *s, u16 location) {
    /* CHDK port: page-table fast path. Most reads - instruction fetch, WRAM,
     * VRAM - are a plain window onto memory, and resolving them here turns a
     * 16-way switch plus bank arithmetic plus assertions into one indexed
     * load. Anything with side effects has a NULL entry and falls through. */
    {
        unsigned char *pg = s->rdmap[location >> 8];
        if (pg)
            return pg[location & 0xff];
    }
    ...
    case 0x3000:
        ...
        /* CHDK port: bank 0 is pinned by the pager, not resident in mem_ROM. */
        return gbc_hook_rom_read(s, 0, location);
    ...
    case 0x7000:
    {
        ...
        u8 bank;
        /* CHDK port: the camera mapper keeps its own bank register. */
        if (s->mbc == GBC_MBC_CAM)
            bank = (u8)gbc_hook_cam_rom_bank(s);
        else {
            bank = s->mem_bank_rom;
            if (s->mbc == 1 && s->mem_mbc1_romram_select == 0)
                bank |= s->mem_mbc1_rombankupper << 5;
        ...
        bank &= s->mem_num_banks_rom - 1;
        return gbc_hook_rom_read(s, bank, location - 0x4000);
    }
    case 0x8000: /* 8000 - 9FFF */
    case 0x9000:
        MMU_DEBUG_R("VRAM");
    ...
    case 0xa000: /* A000 - BFFF */
    case 0xb000:
        /* CHDK port: camera SRAM or the M64282FP register file. */
        if (s->mbc == GBC_MBC_CAM)
            return gbc_hook_cam_read(s, location);
        if (s->mbc == 1) {
            ...
            if (s->mem_mbc3_extram_rtc_select < 0x04)
                ...
                /* CHDK port: -0x08, see the matching write in mmu_write(). */
                return s->mem_RTC[s->mem_mbc3_extram_rtc_select - 0x08];
            else
