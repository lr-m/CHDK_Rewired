...
int rom_get_info(u8 *rom, size_t rom_size, struct rominfo *ret_rominfo) {
    ...
    /* CHDK port: the Game Boy Camera, which is the whole point of this fork.
     * MAC-GBD mapper plus an M64282FP sensor - see modules/gbc/gbc_cam.c.
     * Always has SRAM (128K) and always battery-backed. The persistence that
     * implies is not automatic here - gb_state does not own this cartridge's
     * RAM, gbc_cam_t does - it is implemented in gbc_save.c, which loads the
     * album in gbc_emu_open() and writes it back in gbc_emu_close(). This
     * comment used to claim the photographs survived a power cycle on their
     * own. They did not: the SRAM was a malloc'd buffer that was simply freed,
     * and every picture was lost on exit. */
    case 0xfc: mbc = GBC_MBC_CAM; extram = 1; battery = 1;   break;
    /* MMM01 unsupported */
...
int state_new_from_rom(struct gb_state *s, u8 *rom, size_t rom_size) {
    ...
    /* CHDK port: mem_ROM is deliberately left NULL. Upstream makes the whole
     * cartridge resident, which is 1 MB for the camera and 2 MB for Crystal
     * against a CHDK image that lives in 0x32000 bytes of ARAM. ROM reads go
     * through gbc_hook_rom_read() and the LRU in gbc_rom.c instead; `rom` here
     * is only bank 0, borrowed for the header parse above.
     *
     * The camera cartridge likewise owns its own 128K of SRAM inside
     * gbc_cam_t, so mem_EXTRAM stays NULL for it too. */
    s->mem_ROM = NULL;

    s->mem_WRAM = malloc(WRAM_BANKSIZE * s->mem_num_banks_wram);
    s->mem_VRAM = malloc(VRAM_BANKSIZE * s->mem_num_banks_vram);
    if (!s->mem_WRAM || !s->mem_VRAM)
        err("Out of memory allocating WRAM/VRAM");
    memset(s->mem_WRAM, 0, WRAM_BANKSIZE * s->mem_num_banks_wram);
    memset(s->mem_VRAM, 0, VRAM_BANKSIZE * s->mem_num_banks_vram);

    if (s->mem_num_banks_extram && s->mbc != GBC_MBC_CAM) {
        s->mem_EXTRAM = malloc(EXTRAM_BANKSIZE * s->mem_num_banks_extram);
        if (!s->mem_EXTRAM)
            err("Out of memory allocating cartridge RAM");
        memset(s->mem_EXTRAM, 0, EXTRAM_BANKSIZE * s->mem_num_banks_extram);
    }
...
void init_emu_state(struct gb_state *s) {
    s->emu_state = ucalloc(sizeof(struct emu_state));
 ...
 */
/* CHDK port: save states and the battery-RAM dumper are compiled out from
 * here to the end of the file. They serialise mem_ROM, which this port no
 * longer keeps resident (it is paged off the card), and they reach for
 * read_file/save_file in fileio.c, which is a hosted-stdio file this port does
 * not build. Cartridge RAM persistence for the camera is handled separately by
 * gbc_cam.c, which owns its own SRAM and knows how big it is. */
#ifndef GBC_CHDK_PORT
