...
struct gb_state {
    ...
    /* CHDK port. Opaque here so the vendored headers do not have to know
     * about gbc_rom.h / gbc_cam.h; gbc_emu.c casts them back.
     *   gbc_pager  - gbc_rom_t,  the card-backed ROM window (always present)
     *   gbc_cart   - gbc_cam_t,  only when mbc == GBC_MBC_CAM */
    void *gbc_pager;
    void *gbc_cart;

    /* Read page table: 256 entries of 256 bytes, indexed by the high byte of
     * the address. A non-NULL entry means the read is a plain indexed load;
     * NULL means fall through to the general decode in mmu_read(), which is
     * where anything with side effects lives (I/O, cartridge RAM, echo, OAM).
     * Rebuilt by gbc_mmu_remap() - every bank change must call it. */
    unsigned char *rdmap[256];
};
