...
{
    // -2 rather than -1 so the caller can tell an allocation failure from a
    // failed open. They are different faults with different fixes and both
    // used to arrive as the same "open error".
...
flat_hdr* module_preload(const char *path, const char *name, _version_t ver)
{
    ...
    //
    // Note "< 0", not "<= 0". Zero is a perfectly good file descriptor, and on
    // the a460 - VxWorks - Canon's open() really does hand it out for the module
    // file, because nothing else in that task has a file open at the time. The
    // original "<= 0" threw it away as an error, so *every* module load failed
    // with "open error" on that body while the DryOS ports, which never seem to
    // return 0, were unaffected.
    //
    // The rest of this same file already gets this right: the log writer tests
    // "fd >= 0" and module_get_ptp_size tests "fd < 0". b_open() still returns
    // -1 when its read buffer allocation fails, so that case is still caught.
    int module_fd = b_open(path);
    if (module_fd < 0)
    {
        char errbuf[40];
        if (module_fd == -2)
            strcpy(errbuf, "umalloc(4096) failed");
        else
            sprintf(errbuf, "open()=%d %s", module_fd, path);
        moduleload_error(name, errbuf);
        return 0;
    }
