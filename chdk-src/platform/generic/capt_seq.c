...
void __attribute__((naked,noinline)) capt_seq_hook_set_nr()
{
 ...
 // Same erase as the one kbd_update_key_state() does on the shutter keys, for
 // the shots that no key press starts: self timer, remote, script. Exposure
 // start is still ahead of anything Canon does with the review.
 {
