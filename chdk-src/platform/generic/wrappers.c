...
// JPEG encoder working buffer - see raw_buffer.h. Not reversed on most ports,
// and there is nothing sensible to guess, so the default is "no such buffer".
char __attribute__((weak)) *hook_jpeg_buffer(unsigned *len) {
	*len = 0;
	return 0;
}
