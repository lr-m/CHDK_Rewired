<#
.SYNOPSIS
    Flash an SD card with CHDK for the PowerShot A410 (firmware 100e), on Windows.

.DESCRIPTION
    This is a release script: it lives beside CHDK-A410-100e-card.zip and, with no -Source
...
$cardZip = Join-Path $here 'CHDK-A410-100e-card.zip'
...
$tempSource = $null
if (-not $Source) {
    if (-not (Test-Path $cardZip)) { throw "CHDK-A410-100e-card.zip is missing from $here" }
    $Source = $cardZip
}
