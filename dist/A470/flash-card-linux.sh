#!/usr/bin/env bash
# Flash an SD card with CHDK for the PowerShot A470 (firmware 102c), on Linux.
#
# This is a release script: it lives beside CHDK-A470-102c-card.zip and, given no second
...
if [[ -z "$SRC" ]]; then
    SRC="$HERE/CHDK-A470-102c-card.zip"
    [[ -f "$SRC" ]] || { echo "error: CHDK-A470-102c-card.zip is missing from $HERE" >&2; exit 1; }
fi
