<#
.SYNOPSIS
    Flash an SD card with CHDK for the PowerShot A470 (firmware 102c), on Windows.

.DESCRIPTION
    This is a release script: it lives beside CHDK-A470-102c-card.zip and, with no -Source
...
$cardZip = Join-Path $here 'CHDK-A470-102c-card.zip'
...
$tempSource = $null
if (-not $Source) {
    if (-not (Test-Path $cardZip)) { throw "CHDK-A470-102c-card.zip is missing from $here" }
    $Source = $cardZip
}
