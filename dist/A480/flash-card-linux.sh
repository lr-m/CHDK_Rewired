#!/usr/bin/env bash
# Flash an SD card with CHDK for the PowerShot A480 (firmware 100b), on Linux.
#
# This is a release script: it lives beside CHDK-A480-100b-card.zip and, given no second
...
if [[ -z "$SRC" ]]; then
    SRC="$HERE/CHDK-A480-100b-card.zip"
    [[ -f "$SRC" ]] || { echo "error: CHDK-A480-100b-card.zip is missing from $HERE" >&2; exit 1; }
fi
