<#
.SYNOPSIS
    Flash an SD card with CHDK for the PowerShot A640 (firmware 100b), on Windows.

.DESCRIPTION
    This is a release script: it lives beside CHDK-A640-100b-card.zip and, with no -Source
...
$cardZip = Join-Path $here 'CHDK-A640-100b-card.zip'
...
$tempSource = $null
if (-not $Source) {
    if (-not (Test-Path $cardZip)) { throw "CHDK-A640-100b-card.zip is missing from $here" }
    $Source = $cardZip
}
