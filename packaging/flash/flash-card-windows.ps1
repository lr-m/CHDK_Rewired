<#
.SYNOPSIS
    Flash an SD card with CHDK for the PowerShot @MODEL_UC@ (firmware @FW@), on Windows.

.DESCRIPTION
    This is a release script: it lives beside @CARDZIP@ and, with no -Source
...
$cardZip = Join-Path $here '@CARDZIP@'
...
$tempSource = $null
if (-not $Source) {
    if (-not (Test-Path $cardZip)) { throw "@CARDZIP@ is missing from $here" }
    $Source = $cardZip
}
